...
export function LivingAtmosphere({ className }: { className?: string }) {
  ...
  return (
    <div
      aria-hidden="true"
      className={cn(
        "pointer-events-none absolute inset-0 overflow-hidden bg-cream",
        className
      )}
    >
      ...
      {/* soft-light, not multiply: multiply can only darken, so over a ground
          this light it stopped reading as grain and started reading as a wash
          — it pulled the cream down about five levels on every channel, which
          is exactly the dirtied tan we are avoiding. soft-light lifts and
          sinks in equal measure, so the texture is there and the cream keeps
          its own value. */}
      <div className="film-grain absolute inset-0 mix-blend-soft-light" />
    </div>
  );
}
