...
### The dither — the one exception to the grain rejection

Film grain is on the never-again list and stays there. What follows is not it,
and the difference is the whole justification, so it is stated in numbers
before anything else.

| | film grain (rejected) | the dither (`.ramp-dither`) |
|---|---|---|
| amplitude | ~13 channels (5% opacity over a mid-grey SVG grain) | **1 channel**, the smallest step the screen can express |
| mean absolute contribution | ~6 channels | **0.42–0.59** on every single-layer region, 0.72 where two layers meet |
| cell | a few CSS px, scaled with the page | **one device pixel**, 72 to a tile |
| where | over dark grounds *and photographic frames* | over the darkening ramps only, never a photograph on its own |
| why | to be seen — to kill the flat digital-paint feel | to *not* be seen — to take the alignment off an 8-bit contour |

**The defect.** Every darkening on this site is a long ramp through the dark
end of the scale, and 8 bits has no room down there. Measured at 390×664 on a
device ratio of 3, in the smooth band of the hero's sky, the crossing held one
channel value for **37 device rows** at a stretch and a single one-step jump
held its row across **69** of them — a line drawn across the picture, which is
what the owner reported from an iPhone as faint blue bands travelling through
the crossing. It is not one instance. Every ramp on the page measured the same
way, and so did the graded photograph under them: the sky in `home-hero` bands
on its own over 12 channels before any darkening touches it.

**What the layer does, and what it cannot do.** There is no precision left to
recover — the output is 8 bits and the ramp has to cross the same values
whatever paints it — so the contour is not removed, its **alignment** is. Half
the device pixels, chosen by a 72-cell void-and-cluster blue-noise tile, take a
single step up and the other half take nothing; the staircase survives in the
average and the eye can no longer find its edges. The tile's low-frequency
power is 1.1% of its high-frequency power, which is why the perturbation reads
as nothing rather than as texture.

**Measured, before → after** (the smooth sky band of the crossing, twelve
points through the boundary's travel, worst of four columns):

| engine, viewport | flat run, max | one step held across, p95 | rows over 8px |
|---|---|---|---|
| WebKit 390×664 ×3 | 37 → **11** | 20.3 → **8.5** | 71.5% → **4.4%** |
| WebKit 1440×900 ×2 | 39 → **11** | 23.0 → **8.9** | 84.6% → **7.0%** |
| Chromium 390×664 ×3 | 117 → **12** | 32.2 → **7.4** | 17.7% → **2.2%** |
| Chromium 1440×900 ×2 | 285 → **13** | 73.3 → **8.0** | 21.3% → **4.1%** |

On a flat ground with no ramp under it at all the same instrument reads a max
run of 6–7 and an alignment of 6 — that is the pattern's own statistics, and
the after column is at it. The worst single sample in half a million is the
dither, not a surviving edge.

**The three rules of the layer**, each of which cost a measurement:

1. **It is a sibling of the ramp, never a child.** Inside a masked element the
   pseudo-element's backdrop is the ramp's own semi-transparent gradient, so
   the step lands on an alpha of 0.08–0.92 and is quantized away with it:
   `.plate-shade::after` read 0.5149 on the resting poster and **0.0000** at
   every row of the crossing. `.chrome-shade` and `.foot-shade` are the
   exception and keep a pseudo-element — neither carries a mask.
2. **It may not stand inside a scaled layer.** Inside the hero's push
   (1.0065–1.0254) the tile perturbed 0–3% of the pixels against 46–52%
   outside one, and neither a coarser cell (2, 3, 4 and 6 device pixels were
   measured) nor a counter-scale on the tile recovered it — the compositor
   scales the layer after rasterizing it. So the hero's two copies stand in
   the section and in the frame, the two boxes the push does not reach.
3. **The step is the layer's `opacity` and the tile's own alpha is 0 or 255.**
   An alpha of 1 inside the tile times any filter weight under 1 rounds to
   nothing, so every fractional transform annihilated it. Carried as opacity
   the tile stays a hard binary image, filtering spreads the perturbation
   instead of deleting it, and the compositor multiplies the group's alpha in
   float. `plus-lighter` is required with it: normal compositing of a 1/255
   white is inside WebKit's own rounding and reads exactly 0.0000.

**Cost.** No new pass: through 160 notches of the crossing, headed Chromium
with GPU rasterization counts **1280 raster tasks and 1661 paints on both
builds**, and the raster time moves +0.02 to +0.04 ms per frame against a
run-to-run spread of ±0.06 ms. Headless software raster reads +7.5 ms per
frame, which is the inflation the Earth's note already prices and is recorded
as such, not as the cost. The stylesheet grows 1.4 kB gzipped, all of it the
tile.

**It is never over a glyph, and that is asserted rather than argued.** Every
text node on all four pages, both engines, both viewports, dither on against
dither off, counting only the pixels that *are* the ink: **1,757,537 ink
pixels, 0 moved.**

**What it costs the contrast ledger, and this is general.** A layer that raises
half the device pixels of a ground by one channel raises the worst pixel every
contrast measurement looks for, so every reading on the site falls by about a
channel's worth: over 158 elements on four pages at two viewports, a median of
**−0.07** of a ratio point, p05 −0.15, worst −0.23. The below-floor set is
unchanged in membership — twelve before, twelve after — and one element that
sat **exactly** on its floor crossed it and was put back: `/contact/`'s
`Email:` label at 390×664, 4.50 → 4.46, restored to **4.57** by taking the
hero plate's shade base from 63 to 66 (its worst pixel is at 94–97% of the
window with that plate at 0.65 of the crossfade, so the channels plate's mid
is not the lever and 68 there moved it not at all). Darkening only ever raises
a cream reading, so the ratchet is satisfied on the rest of that plate rather
than traded against. **Any future dither, anywhere, owes the ledger the same
re-measurement.**

**Where it is not.** `.gold-field` carries no dither because it paints no pixel
on the built page — the hero's photograph covers its top edge and its bottom
edge is the `open` variant. And where a window-pinned ramp lies over a plate —
the top 130/150 px under the chrome, the footer's last band on an inner page —
two layers land on one pixel and the step is two rather than one: mean 0.72,
11% of pixels at 2. That is accepted rather than solved. Two independent ±1
